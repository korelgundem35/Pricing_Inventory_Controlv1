...
def run_combined_algorithms(trained_on_optimal):
    """
    ...
    if trained_on_optimal==True:
        ...
        df_fqi.to_excel('outputs/output_fqi_optimal.xlsx', index=False)

        df_pfqi = pd.DataFrame(rewards_dic_pess)
        df_pfqi.to_excel('outputs/output_pfqi_optimal.xlsx', index=False)
    else:
        df_fqi = pd.DataFrame(rewards_dic)
        df_fqi.to_excel('outputs/output_fqi_uniform.xlsx', index=False)

        df_pfqi = pd.DataFrame(rewards_dic_pess)
        df_pfqi.to_excel('outputs/output_pfqi_uniform.xlsx', index=False)
